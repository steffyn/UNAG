...
#Incluida Katherine
@permission_required('alumnos.add_alumnos', login_url='/censo/logout/')
def registro_alumno_excel2(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			guardar.usuario_creador = User.objects.get(id=request.user.id)
			guardar.save()

			# with rarfile.RarFile(guardar.archivo.path) as rf: #abre el rar
			#     with rf.open('Libro1.xlsx') as f: #se coloca en el archivo de excel para abrirlo
	    	#book = xlrd.open_workbook(file_contents=f.read()) #abre el archivo de excel
	    	
	    	book = xlrd.open_workbook(file_contents=guardar.archivo.read()) #abre el archivo de excel
	    	sh = book.sheet_by_index(0) #toma la primera hoja de excel
	    	row_comienza = 10 #La fila donde comienzan los datos
	    	row_termina = 10#49 #la fila donde terminan los datos

	    	carrera = sh.cell_value(3, 2)
	    	periodo = sh.cell_value(4, 2)
	    	clase = sh.cell_value(5, 2)

    		while (row_comienza <= row_termina): #recorre todas las filas
	    		...
	    		identidad = sh.cell_value(row_comienza, 1)
	    		...
	    		genero = sh.cell_value(row_comienza, 6)
	    		civil = sh.cell_value(row_comienza, 7)
	    		direccion = sh.cell_value(row_comienza, 8)
	    		correo_electronico = sh.cell_value(row_comienza, 9)
	    		celular = sh.cell_value(row_comienza, 10)
	    		telefono_fijo = sh.cell_value(row_comienza, 11)
	    		...
	    		centro_estudios = sh.cell_value(row_comienza, 17)
	    		...
	    		posicion_familiar = sh.cell_value(row_comienza, 28)
	    		observaciones = sh.cell_value(row_comienza, 29)

	    		usuario = User()
	    		usuario.username = identidad
	    		usuario.save()
	    		usuario.set_password('123')
	    		usuario.save()

	    		personas = persona() 
	    		personas.identidad = identidad
	    		personas.tipo_identificacion = tipo_identificacion.objects.get(descripcion=str(identificacion))
	    		...
	    		personas.estado_civil = estado_civil.objects.get(descripcion=civil)
	    		personas.direccion = direccion 
	    		personas.celular = smart_str(int(telefono_fijo))
	    		personas.telefono_fijo = smart_str(int(celular))
	    		personas.pais_nacimiento = pais.objects.get(descripcion=pais_nacimiento)
	    		personas.pais_residencia = pais.objects.get(descripcion=pais_residencia)
	    		personas.tipo_persona = tipo_persona.objects.get(descripcion=TipoPersona)
	    		personas.zona = zona.objects.get(descripcion=Zona)
	    		personas.usuario = usuario

	    		personas.usuario_creador = User.objects.get(id=request.user.id)
	    		personas.usuario_modificador = User.objects.get(id=request.user.id)
	    		personas.save()

	    		lista_titulos = titulos.split(",")
	    		lista_centros = centro_estudios.split(",")	    		

	    		try:
		    		for x in lista_titulos:
		    			personas.titulos.add(Titulos.objects.get(pk=int(smart_str(unicode(x)))))		    			
	    		except Exception, e:
					print "E_2",e

	    		for x in lista_centros:
	    			personas.centros.add(centro.objects.get(descripcion=unicode(x)))
	    		
	    		personas.save()

	    		alumno = alumnos()
	    		alumno.persona = personas
	    		alumno.nombre_padre = nombre_padre
	    		alumno.profesion_padre = profesion_padre
	    		alumno.telefono_padre = telefono_padre
	    		
	    		if asociacionPadre != "":
	    			alumno.asoc_campesina_padre = asoc_campesina.objects.get(descripcion=asociacionPadre)

	    		alumno.nombre_madre = nombre_madre
	    		alumno.profesion_madre = profesion_madre
	    		alumno.telefono_madre = telefono_madre
	    		
	    		if asociacionPadre != "":
	    			alumno.asoc_campesina_madre = asoc_campesina.objects.get(descripcion=asociacionMadre)
	    		alumno.correo_electronico = correo_electronico
	    		alumno.tiene_hijos = tiene_hijos
	    		alumno.observaciones = observaciones
	    		alumno.promedio_graduacion = promedio_graduacion
	    		alumno.codigo_registro = codigo_registro
	    		alumno.usuario_creador = User.objects.get(id=request.user.id)
	    		alumno.usuario_modificador = User.objects.get(id=request.user.id)
	    		alumno.posicion_familiar = posicion_familiar
	    		alumno.save()



    			# while (col_comienza <= col_termina): #recorre todas las columnas por fila
	    		# 	valor = sh.cell_value(row_comienza, col_comienza)

	    		# 	col_comienza += 1
	    		# 	print 'ROW----->', row_comienza, col_comienza, valor
	    		row_comienza += 1
	else:
		...
		if form.is_valid():
    		...
    		if flag == True:
	    		...
	    		for row in range(row_comienza, sh.nrows): #recorre todas las filas
		    		...
		    		print 'IMPRIME EL ESTADO CIVIL:', Estado_civil
		    		print Estado_civil
		    		if Estado_civil == "":
		    			flag = False
	    			...
	    			print 'Fecha de nacimiento',pydatetime
		    		...
		    		if flag == False:
	    				...
	    				registro_persona, created = persona.objects.update_or_create(identidad=identidad, 
	    					defaults=query_arg)
